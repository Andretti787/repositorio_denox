    ...
    try:
         ...
         with pyodbc.connect(connection_string, timeout=5) as conn:
            ...
            -- Convertimos la fecha a formato YYYY-MM-DD directamente en la consulta
             SELECT TOP 100 '72761617' AS ID_CLIENTE, DEL.BPDNAM_0 AS NOMBRE_CONSIGNATARIO, SUBSTRING(CONCAT(BPDADDLIG_0, BPDADDLIG_1, BPDADDLIG_2),1,30) AS DIR_CONSIG
            ...
            , 180 AS LARGO, 90 AS ANCHO, 120 AS ALTO, 'CM' AS UD_MEDIDA, VOL_0 as VOLUMEN, VOU_0 AS UD_VOLUMEN
			, CASE (SUBSTRING(TEX.TEXTE_0,1,1)) WHEN '{' THEN ''     
                                                ELSE SUBSTRING(REPLACE(REPLACE(TEX.TEXTE_0 ,CHAR(13),' '), CHAR(10),' '),1,149) END AS OBSERVACIONES   
            , RIGHT(ADR.TEL_0,9) AS TFNO
			...
			LEFT JOIN LIVE.BPADDRESS ADR ON ADR.BPANUM_0 = DEL.BPCORD_0 AND ADR.BPAADD_0 = DEL.BPAADD_0 
            WHERE DEL.BPTNUM_0 = '025'
            ORDER BY SHIDAT_0 DESC, SDHNUM_0 DESC
             """
